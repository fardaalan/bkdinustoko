...
class Shop extends BaseController
{
	...
	private $apiKey = "c15f925839dffea77b57279e400ae3fd";
	...
	public function getVoucher()
	{
		...
		$voucher = $voucherModel->select('*')->where('kode_voucher', $voucher)->get()->getRow();
		if($voucher){
			if($voucher->aktif == 0){
				$data['status'] = 'error';
				$data['msg'] = 'Status voucher tidak aktif !';
			}else{
				if($voucher->tanggal_mulai_berlaku <= $tanggal_sekarang && $tanggal_sekarang <= $voucher->tanggal_akhir_berlaku){
					$data['status'] = 'success';
					$data['msg'] = 'Voucher berhasil digunakan !';
					$data['nominal'] = $voucher->besar_diskon;
				}else{
					$data['status'] = 'error';
					$data['msg'] = 'Voucher kadaluarsa !';
				}
			}
			// return $voucher->besar_diskon;
		}else{
			$data['status'] = 'error';
